		...
		if (empty($server->tokenname) || empty($server->tokenvalue)) {
			...
			switch ($foundgroups) {
				case 0:
					...
					$proxmox->post("/access/groups",['groupid' => $groupid, 'comment' => 'fossbilling group']);
					break;
				case 1:
			...
			foreach ($users as $user) {
				...
				switch ($found) {
					case 0:
						...
						$proxmox->post("/access/users", ['userid' => $userid, 'password' => $this->di['tools'], 'enable' => 1, 'comment' => 'fossbilling user', 'groups' => $groupid]); /* @phpstan-ignore-line */
